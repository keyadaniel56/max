...
/**
 * Provider wrapper for bot-builder routes.
 * Renders the full auth/store/provider stack plus the app Layout.
 * Child routes (AppRoot) are rendered inside Layout via <Outlet />.
 */
const BotBuilderShell = () => (
    <Suspense
...
const router = isPreviewMode()
    ? createBrowserRouter(
          createRoutesFromElements(
              <Route path='/' element={<BotBuilderShell />}>
    ...
    : createBrowserRouter(
          createRoutesFromElements(
              <>
                  {/* Landing page — public, no auth providers */}
                  <Route
                      path='/'
                      element={
                  ...
                  />
                  {/* Bot builder — auth required, full provider stack */}
                  <Route path='botbuilder' element={<BotBuilderShell />}>
                      <Route index element={<AppRoot />} />
                  </Route>
                  {/* Preview route (same shell as botbuilder) */}
                  <Route path='preview' element={<BotBuilderShell />}>
                      <Route index element={<AppRoot />} />
                  </Route>
              </>
          )
        ...
        const handleCallback = async () => {
            try {
                ...
                if (accounts && accounts.length > 0) {
                    ...
                    window.location.replace('/botbuilder');
                } else {
